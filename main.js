...
function preload() {
    yay = loadSound('ball_touch_paddel.wav');
    no = loadSound('missed.wav');
...
function gotPoses(results) {
	if (results.length > 0) {
		lWx = results[0].pose.nose.x;
		lwY = results[0].pose.nose.y;
	}
...
function paddleInCanvas(){
  if(lwY+paddle1Height > height){
    lwY=height-paddle1Height;
  }
  if(lwY < 0){
    lwY =0;
  }  
